...
// Custom Type Definition Extension for user validation mapping
...
app.get("/events", async (req: AuthenticatedRequest, res: express.Response) => {
  try {
...
app.get("/events/:id", async (req: express.Request, res: express.Response) => {
  try {
...
app.post("/events", authenticateToken, async (req: AuthenticatedRequest, res: express.Response): Promise<void> => {
...
app.put("/events/:id", authenticateToken, async (req: AuthenticatedRequest, res: express.Response): Promise<void> => {
  if (req.user?.role !== "organizer") {
    res.status(403).json({ error: "Forbidden: Organizer role required" });
    return;
  }
...
app.delete("/events/:id", authenticateToken, async (req: AuthenticatedRequest, res: express.Response): Promise<void> => {
  if (req.user?.role !== "organizer") {
    res.status(403).json({ error: "Forbidden: Organizer role required" });
    return;
  }
...
app.get("/bookings", authenticateToken, async (req: AuthenticatedRequest, res: express.Response) => {
  try {
...
app.post("/bookings", authenticateToken, async (req: AuthenticatedRequest, res: express.Response): Promise<void> => {
...
app.put("/bookings/:id/cancel", authenticateToken, async (req: AuthenticatedRequest, res: express.Response): Promise<void> => {
  const id = req.params.id as string;
